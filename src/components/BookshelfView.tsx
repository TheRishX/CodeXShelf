...
export function BookshelfView({ dbState, onUpdateDb, onSelectView }: BookshelfViewProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-50 dark:bg-slate-900/40 p-4 lg:p-7 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto mb-6 bg-white dark:bg-slate-950/25 border border-slate-200 dark:border-slate-850 p-4 rounded-3xl flex flex-col xl:flex-row items-center justify-between gap-4 shadow-sm">
        ...
        <div className="relative w-full xl:max-w-md">
          ...
          <input
            ...
            className="w-full text-xs bg-slate-100/50 hover:bg-slate-100 dark:bg-slate-900/60 dark:hover:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-2.5 pl-10 text-slate-800 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 transition-all font-sans"
          />
          {searchTerm && (
            <button
        ...
        {/* Actions Sorting and Switching views */}
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full xl:w-auto xl:justify-end">
          
          {/* Sorter Selector Dropdown */}
          <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-900/80 p-1.5 px-3 rounded-2xl border border-slate-150 dark:border-slate-800 w-full sm:w-auto">
            <ArrowUpDown className="w-3.5 h-3.5 text-amber-500 shrink-0" />
            <select
              value={sortBy}
              onChange={(e) => handleSetSortBy(e.target.value as any)}
              className="bg-transparent text-[11px] font-mono text-slate-600 dark:text-slate-300 focus:outline-none cursor-pointer w-full sm:w-auto uppercase font-bold"
            >
          ...
          {/* View triggers */}
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-1 rounded-2xl justify-center w-full sm:w-auto">
            {[
              ...
              <button
                ...
                className={`px-2.5 py-1.5 rounded-xl text-[10px] font-mono font-bold uppercase tracking-wider transition-all cursor-pointer ${
                  activeTab === tab.id
